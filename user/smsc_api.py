...
SMSC_LOGIN = "gt.bps.by@gmail.com" # логин клиента
SMSC_PASSWORD = "kBgrjdcrFz9"   # пароль
...
class SMSC(object):
    ...
    # id - идентификатор сообщения. Представляет собой
    # 32-битное число в диапазоне от 1 до 2147483647.
    # format - формат сообщения (0 - обычное sms, 1 - flash-sms,
    # 2 - wap-push, 3 - hlr, 4 - bin, 5 - bin-hex, 6 - ping-sms, 7 - mms, 8 - mail, 9 - call)
    # sender - имя отправителя (Sender ID). Для отключения
    # Sender ID по умолчанию необходимо в качестве имени
    ...
    def send_sms(self, phones, message,
        ...
        message = f"cost=3&phones={quote(phones)}&mes={quote(message)}" + \
            f"&translit={str(translit)}&id={str(id)}{ifs(format>0, f'&{formats[format-1]}', '')}" + \
            f"{ifs(sender==False, '', '&sender={quote(str(sender))}')}" + \
            f"{ifs(time, f'&time={quote(time)}', '')}{ifs(query, f'&{query}', '')}"
        
        m = self._smsc_send_cmd("send", message)

        if SMSC_DEBUG:
            if m[1] > "0":
                print(
                    "Сообщение отправлено успешно. ID: {m[0]}," + \
                    " всего SMS: {m[1]}, стоимость: {m[2]}, баланс: {m[3]}"
                    )
            else:
                print(f"Ошибка №{m[1][1:]}{ifs(m[0]>'0', f', ID: {m[0]}', '')}")

        return m


    # SMTP версия метода отправки SMS
    def send_sms_mail(self, phones,
                      message, translit=0,
                      time="", id=0,
                      format=0, sender=""):
        ...
        server.sendmail(
            SMTP_FROM,
            "send@send.smsc.ru",
            f"Content-Type: text/plain; charset={SMSC_CHARSET}\n\n" + \
            f"{SMSC_LOGIN}:{SMSC_PASSWORD}:{id}:{time}:{translit}," + \
            f"{format},{sender}:{phones}:{message}"
        )
        server.quit()
    ...
    # query - строка дополнительных параметров, добавляемая в
    # URL-запрос ("list=79999999999:Ваш пароль: 123\n78888888888:Ваш пароль: 456")
    #
    # возвращает массив (<стоимость>, <количество sms>) либо массив (0, -<код ошибки>) в случае ошибки

    def get_sms_cost(self, phones, message, translit=0,
                     format=0, sender=False, query=""):
        formats = [
            "flash=1",
            "push=1",
            "hlr=1",
